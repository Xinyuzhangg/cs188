        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                if (x, y) == self.corners[0]:
                    successors.append([(nextx, nexty, True, state[3],
                                        state[4], state[5]), action, 1])
                elif (x, y) == self.corners[1]:
                    successors.append([(nextx, nexty, state[2], True,
                                        state[4], state[5]), action, 1])
                elif (x, y) == self.corners[2]:
                    successors.append([(nextx, nexty, state[2], state[3],
                                        True, state[5]), action, 1])
                elif (x, y) == self.corners[3]:
                    successors.append([(nextx, nexty, state[2], state[3],
                                        state[4], True), action, 1])
                else:
                    successors.append([(nextx, nexty, state[2], state[3],
                                        state[4], state[5]), action, 1])
...
def cornersHeuristic(state, problem):
    ...
    if False not in state:
        ...
        if not state[i]:
            heuristic += (abs(xy[0] - corners[i - 2][0]) + abs(xy[1] - corners[i - 2][1]))"""
    """if True not in state:
        heuristic += min(distance) + min(width, height) * 2 + max(width, height)
    elif state.count(True) == 1:
        heuristic += min(distance) + height + width
    elif state.count(True) == 2:
        heuristic += min(distance) + min(width, height)
    else:
        heuristic += distance.pop()"""

    """if xy[0] > 1 and xy[1] > 1 and width - xy[0] > 1 and height - xy[1] > 1:
        for i in range(-1, 2):
            for j in range(-1, 2):
                heuristic += walls[xy[0] - i][xy[1] - j]
